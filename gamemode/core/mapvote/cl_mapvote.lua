...
net.Receive("RAM_MapVoteStart", function()
    ...
    for i = 1, amt do
        ...
        CurrentMaps[#CurrentMaps + 1] = map
    end
    
    EndTime = CurTime() + net.ReadUInt(32)
    
    if(IsValid(Panel)) then
        Panel:Remove()
    end
    
    Panel = vgui.Create("RAM_VoteScreen")
    Panel:SetMaps(CurrentMaps)
end)
...
net.Receive("RAM_MapVoteUpdate", function()
    ...
    elseif(update_type == UPDATE_WIN) then      
        if(IsValid(Panel)) then
            Panel:Flash(net.ReadUInt(32))
        end
    end
end)
    ...
    for k, v in pairs(self.Voters) do
        if(not IsValid(v.Player)) then
            ...
            if(not Votes[v.Player:SteamID()]) then
                ...
                if(HasExtraVotePower(v.Player)) then
